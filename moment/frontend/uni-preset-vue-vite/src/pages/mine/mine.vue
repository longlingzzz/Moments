...
export default {
  ...
  onLoad() {
    ...
    // 监听刷新我的动态事件
    this.__refreshMyPostsHandler = () => {
      this.fetchMyPosts()
    }
    uni.$on('refreshMyPosts', this.__refreshMyPostsHandler)
  ...
  onUnload() {
    ...
    if (this.__refreshMyPostsHandler) {
      uni.$off('refreshMyPosts', this.__refreshMyPostsHandler)
      this.__refreshMyPostsHandler = null
    }
  ...
  methods: {
    ...
    async fetchMyPosts() {
      ...
      try {
        ...
        // 处理后端返回的格式：{success: true, data: {posts: [...], total: ...}}
        if (response && response.success && response.data && Array.isArray(response.data.posts)) {
          ...
          // 更新统计信息
          this.myStats.posts = response.data.total || posts.length
        } else {
          console.error('获取我的动态失败：', response)
          this.myPosts = []
          this.myStats.posts = 0
        }
      } catch (error) {
